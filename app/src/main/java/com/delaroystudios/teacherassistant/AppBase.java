...
public class AppBase extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String q = "SELECT * FROM "+tbl_class.TABLE_NAME;
        Cursor cursor=handler.execQuery(q);
        if(cursor!=null && cursor.moveToFirst()){
            do{
                String buffer = cursor.getString(tbl_class.COL_NM_INDEX);
                if(buffer != null){
                    divisions.add(buffer);
                }
            }
